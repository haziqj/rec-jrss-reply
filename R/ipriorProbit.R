...
## ---- variational.bayes ----
iprobit <- function(y, X, kernel = "Canonical", niter = 100, silent = FALSE) {
  if (kernel == "FBM") H <- iprior::fnH3(X)
  else H <- iprior::fnH2(X)
  H2 <- H %*% H
  if (!silent) pb <- txtProgressBar(min = 0, max = niter - 1, style = 3)
  n <- length(y)

  # Set up parameter results
  lower.bound <- lambda <- rep(NA, niter)
  error.rate <- alpha <- rep(0, niter)
  w <- matrix(NA, ncol = n, nrow = niter)
  ystar <- matrix(NA, ncol = n, nrow = niter)
  ...
  for (t in 1:(niter - 1)) {
    ...
    # Lower bound
    lower.bound[t] <- (n + 2 - log(n)) / 2 + log(2 * pi) +
...
print.ipriorProbit <- function(x, newdata = NULL, testdata = NULL) {
  ...
  else y.hat <- predict(x, newdata)$y
  if (is.null(newdata)) y <- x$y
  else y <- testdata
  error.rate <- sum(y.hat != y) / n
  cat(test.or.train, "error rate:", format(round(error.rate * 100, 2), nsmall = 2), "%")
}
